...
describe("3: FlashSwap", function () {
  ...
  beforeEach(async () => {
    ...
    fundAmount = 11000n * 10n ** 18n; // 20000 dai
    borrowAmount = 10000n * 10n ** 18n; // 10000 dai
    fee = 3n * 10n ** 18n; // 3 dai

    const Flashloan = await ethers.getContractFactory("FlashSwapV2");
  ...
  describe("Borrow Dai", () => {
    it("test flashloan", async () => {
      ...
      await flashloan.testFlashSwap(dai.address, borrowAmount);
      console.log(
        "+++++++++++++++++++++++Complete+++++++++++++++++++++++++++++"
      );
      ...
      await flashloan.withdraw(dai.address);

      console.log(
        "After withdraw: DAI Balance of user",
        ethers.utils.formatEther(await dai.balanceOf(accounts[0].address))
      );
    });
  });
